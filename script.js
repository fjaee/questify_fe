      ...
      ">
        <div style="display: flex; align-items: flex-start; gap: 15px;">
          ...
          <div style="height: 24px; display: flex; align-items: center; gap: 5px;">
            <div class="quest-checkbox" style="--check-color: ${typeColors[quest.type]}; width: 18px; height: 18px;">
...
function addQuestForm() {
  ...
  newForm.innerHTML = `
              ...
              <div class="quest-options" style="display: flex; gap: 12px; flex-wrap: wrap; margin-top: 1rem;">
...
function createQuest(button) {
  ...
  const doDate = form.querySelector(".do-date-input").value
  const dueDate = form.querySelector(".due-date-input").value
  const questType = form.querySelector(".quest-type-input").value
  const guild = form.querySelector(".guild-input").value
  ...
  form.querySelector(".do-date-input").value = ""
  form.querySelector(".due-date-input").value = ""
  form.querySelector(".quest-type-input").selectedIndex = 1 // default to side quest
  form.querySelector(".guild-input").selectedIndex = 0
  form.querySelector(".assignee-input").selectedIndex = 0
...
function triggerDoDatePicker() {
  const input = document.getElementById("doDateInput");

  input.click(); // trigger native datepicker

  input.onchange = () => {
    const date = input.value;
    if (date) {
      alert(`Do date set to: ${formatDate(date)}`);
    }
  };
}